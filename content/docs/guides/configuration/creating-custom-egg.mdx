...
![Egg Process Management](/community/config/eggs/Pterodactyl_Create_New_Egg_Process_Management.png)
...
_Docker images must be specifically designed to work with Pterodactyl Panel._ You should read more about that in our [Creating a Docker Image](/guides/configuration/creating-custom-image) guide.
...
Logs are completely handled by the daemon now and use the docker logs to output the complete output from the server. This can be set like below.
...
![Copy Settings From](/community/config/eggs/Pterodactyl_Create_New_Egg_Copy_Settings_From.png)
...
![Egg Variables](/community/config/eggs/Pterodactyl_Create_New_Egg_Variables.png)
...
![Egg Startup](/community/config/eggs/Pterodactyl_Create_New_Egg_Startup.png)
