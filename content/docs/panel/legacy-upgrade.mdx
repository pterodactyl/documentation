...
* PHP `8.0` or `8.1` (recommended) with the following extensions: `cli`, `openssl`, `gd`, `mysql`, `PDO`, `mbstring`,
  ...
  for [Upgrading PHP](/guides/configuration/php-upgrade) for details.
...
are not always detected properly, so simply unpacking over this location will result in some confusing behavior.
